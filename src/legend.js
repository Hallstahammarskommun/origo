...
function addTickListener (layer) {
  ...
  $('#' + name).on('click', function(evt) {
    if ($(evt.target).closest('div').hasClass('o-icon-expand')) {
      toggleGroup($(this));
    } else if (secure === false || secure === undefined) {
      $(this).each(function() {
        var that = this;
...
function addLegend(groups) {
  ...
  layers.forEach(function(layer) {
    ...
    if(layer.get('group') == 'background') {
      ...
      item = '<li class="o-legend ' + name + '" id="' + name + '"><div class ="o-legend-item"><div class="o-checkbox"><svg class="o-icon-fa-check"><use xlink:href="#fa-circle"></use></svg></div>';
      item += title;
